...
Memory allocation is performed at run time since the sizes of contexts used by cryptographic
primitives are unknown until the protocol is selected.
A static stack-like structure is used instead of `malloc`.
...
3. Process the byte per the current state of the state machine. Call output handlers as necessary.
4. Exit `molePutc`.
