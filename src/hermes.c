...
// The payload has been authenticated, temp is the data length
        ...
        case HERMES_TAG_ACK:
            ...
            PRINTF("\nReceived ACK=%d, port=%p; ", ctx->rxbuf[0], (void*)ctx);
...
endcase:
    if ((r) && (ctx->tag == HERMES_TAG_MESSAGE)) {
